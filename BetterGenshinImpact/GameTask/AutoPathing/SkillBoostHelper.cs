...
public partial class PathExecutor
{
    ...
    private Func<double, WaypointForTrack, double, ImageRegion, int, Task<bool>> CreateHurryOnHandler()
    {
        var state = new HurryOnState();
        return (diff, waypoint, distance, screen, num) =>
            TryHurryOnAsync(diff, waypoint, distance, screen, num, state);
    }
